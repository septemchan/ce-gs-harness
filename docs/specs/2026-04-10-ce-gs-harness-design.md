...
- **CE** 负责代码项目开发全链：ce:brainstorm → ce:plan → ce:work → ce:review → ce:compound
- **Superpowers** 负责非代码项目全链：brainstorming → writing-plans → executing-plans
- **gstack** 负责外部世界：/qa、/browse、/ship、/land-and-deploy、/canary、/cso

### 起点决定链路

不按项目类型自动路由，由用户选择起点决定后续链路：

- `/ce:brainstorm` → `/ce:plan` → `/ce:work` → `/ce:review` → `/ce:compound`（代码项目）
- `/superpowers:brainstorming` → `writing-plans` → `executing-plans`（非代码项目）

CE 的构思和计划阶段支持非代码话题（有 universal-brainstorming/planning 备选路径），但执行和审查阶段是纯代码的（跑测试、commit、PR、17 个代码 reviewer）。Superpowers 全链路通用，不区分代码和非代码。
...
Superpowers 仍然安装（用于非代码项目手动调用），但其 SessionStart hook 会在每次新会话注入 118 行导航规则，与 harness 的 workflow-map rule 冲突。安装 ce-gs-harness 后需要禁用 Superpowers 的 SessionStart hook。

Superpowers 的所有 skill（brainstorming、writing-plans、executing-plans、TDD、debugging、verification 等）仍可手动调用，不受影响。非代码项目通过手动调用 /superpowers:brainstorming 启动 Superpowers 链路。
...
**改写路由表：** 去掉现有的无条件路由表和默认路由表，替换为精简版：

```
# 插件分工

起点决定链路：
- 代码项目 → /ce:brainstorm 起步（后续 CE 自动接力）
- 非代码项目 → /superpowers:brainstorming 起步（后续 Superpowers 自动接力）

gstack 负责外部世界（QA/发布/部署/监控/安全）
ce-gs-harness 负责基础设施（项目上下文/阶段导航/健康度）
```

路由原则只在 CLAUDE.md 写 2 行（起点 → 链路），完整的阶段导航细节由 workflow-map rule 提供，不重复。
...
```
起点决定链路：用户选择起点，后续链路自动跟随
  代码项目       → /ce:brainstorm 起步，走 CE 链
  非代码项目     → /superpowers:brainstorming 起步，走 Superpowers 链
...
阶段 2：构思
  明确要做什么                      → /superpowers:brainstorming
                                      ↓ Superpowers 自动建议 writing-plans

阶段 3：规划
  实施计划                          → /superpowers:writing-plans
  多视角计划审查                    → gstack /plan-ceo-review, /plan-eng-review, /plan-design-review
                                      ↓ Superpowers 自动建议 executing-plans
